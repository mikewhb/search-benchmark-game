...
def execute_fts_query(table, query_str, limit):
    ...
    对于 AND 查询，需要使用 fts_search() 或 SQL 方式
    """
    ...
    if query_type == "phrase":
        ...
        return table.search(terms, query_type="fts").phrase_query(True).select(["id", "_score"]).limit(limit)
    
    elif query_type == "intersection":
        # Intersection (AND) 查询
        # LanceDB 默认是 OR，我们需要找到包含所有词的文档
        # 方法1: 使用 BooleanQuery 风格的查询（如果支持）
        # 方法2: 使用 SQL filter 进行后过滤（性能较差但保证正确性）
        # 
        # 根据文档，可以尝试使用 full_text_search 函数的高级选项
        # 这里我们先使用基本的 OR 查询，然后说明限制
        return table.search(terms, query_type="fts").phrase_query(False).select(["id", "_score"]).limit(limit)
        ...
        # Term 查询: 单个词
        return table.search(terms, query_type="fts").select(["id", "_score"]).limit(limit)
